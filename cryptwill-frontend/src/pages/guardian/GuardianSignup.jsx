...
import { Shield, Mail, Lock, Eye, EyeOff, UserPlus, Flower2 } from 'lucide-react';
...
export default function GuardianSignup() {
  ...
  const handleSubmit = async (event) => {
    ...
    setLoading(true);
    try {
      const res = await api.post('/guardians/signup', {
      ...
      toast.success(
        res.data.data?.pendingInviteCount
          ? `Account created — ${res.data.data.pendingInviteCount} invitation(s) waiting`
          : 'Guardian account created'
      );
      navigate('/guardian');
    } catch (err) {
      toast.error(err.response?.data?.error || 'Could not create guardian account');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div
      className="relative min-h-screen overflow-hidden flex items-center justify-center py-10"
        ...
        >
          ...
          <div className="p-8 pt-7">
            ...
            >
              ...
              >
                Register with your email to receive guardian invitations from estate owners in your portal.
              </motion.p>
            </motion.div>
            ...
            >
              ...
              {/* Confirm Password */}
              <motion.div
                animate={{ scale: focused === 'confirmPassword' ? 1.01 : 1 }}
                transition={{ duration: 0.2 }}
              >
                <label className="block text-xs font-semibold mb-1.5 tracking-wide uppercase" style={{ color: '#9a7060' }}>
                  Confirm Password
                ...
                <div className="relative">
                  <Lock
                    className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 transition-colors duration-300"
                    style={{ color: focused === 'confirmPassword' ? '#c8956b' : '#b8a090' }}
                  />
                  <input
                    type={showPassword ? 'text' : 'password'}
                    ...
                    className={inputBase}
                    style={{
                      borderColor: focused === 'confirmPassword' ? '#c8956b' : 'rgba(200,149,107,0.3)',
                      boxShadow: focused === 'confirmPassword' ? '0 0 0 4px rgba(200,149,107,0.12)' : 'none',
                    }}
                  />
                </div>
              </motion.div>
